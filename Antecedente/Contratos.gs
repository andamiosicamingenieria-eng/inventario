...
const CELDAS = {
  ...
  IVA_PORCENTAJE: 'F60',
  PAGARE: 'K156',
  PAGARE_LETRA: 'C168', // <-- Nueva celda agregada para la cantidad con letra
...
function guardarContrato(formData) {
  ...
  try {
    ...
    // 1.5. Calcular y colocar la cantidad del pagaré con letra en C168
    const montoPagare = Number(formData.pagare) || 0;
    if (montoPagare > 0) {
      const textoLetra = numeroALetras(montoPagare);
      ws.getRange(CELDAS.PAGARE_LETRA).setValue(textoLetra);
    } else {
      ws.getRange(CELDAS.PAGARE_LETRA).setValue('');
    }

    // 2. Forzar recálculo de VLOOKUPs y fórmulas en la hoja antes del PDF
    ...
    // 5. Generar PDF (ya incluirá la celda C168 llena)
    let pdfUrl = '';
    try {
...
function escribirEnPlantilla(ws, d) {
  ...
  set(CELDAS.CLIENTE_ID, d.cliente); 
...
function leerDatosResueltos(ws, formData) {
  ...
  return {
    ...
    cliente: String(get(CELDAS.CLIENTE_NOMBRE) || ''), 
...
function guardarEnDataContratos(ss, datos, formData) {
  ...
  row[1] = datos.numero_contrato;          
  row[3] = formData.renta_anterior || '';   
  row[4] = formData.renta_posterior || '';  
  row[5] = new Date();                     
  row[6] = datos.cliente;                  
  row[7] = datos.dias_renta;               
  row[8] = datos.fecha_inicio_renta;       
  row[9] = datos.fecha_fin_renta;          
  row[13] = datos.total_contrato;          
  row[24] = datos.sucursal;                
  row[26] = datos.cantidad_total;          
  row[29] = datos.vendedor;                
  row[30] = datos.vendedor;                
  row[31] = datos.medio_contacto;          
...
function generarPDF(ss, ws, datos) {
  ...
  // Construir URL de exportación PDF (Modificado fzr=true)
  const pdfUrl = 'https://docs.google.com/spreadsheets/d/' + ssId + '/export?' +
    ...
    '&printtitle=false&sheetnames=false&pagenum=false&fzr=true' + // <-- Cambiado aquí
    '&gid=' + sheetId;
...
function limpiarCeldas(ws) {
  ...
  clear(CELDAS.OTROS);
  clear(CELDAS.PAGARE);
  clear(CELDAS.PAGARE_LETRA); // <-- Limpia también la celda de letras
...
// ═══════════════════════════════════════════════════════════════
// FUNCIÓN AUXILIAR: CONVERTIR NÚMEROS A LETRAS (MONEDA MXN)
// ═══════════════════════════════════════════════════════════════
function numeroALetras(num) {
  const unidades = (n) => ['===', 'UN', 'DOS', 'TRES', 'CUATRO', 'CINCO', 'SEIS', 'SIETE', 'OCHO', 'NUEVE'][n];
  const decenas = (n) => {
    let d = ['DIEZ', 'VEINTE', 'TREINTA', 'CUARENTA', 'CINCUENTA', 'SESENTA', 'SETENTA', 'OCHENTA', 'NOVENTA'][n - 1];
    return d || '';
  };
  const especiales = (n) => ['ONCE', 'DOCE', 'TRECE', 'CATORCE', 'QUINCE', 'DIECISEIS', 'DIECISIETE', 'DIECIOCHO', 'DIECINUEVE'][n - 11];
  const centenas = (n) => ['===', 'CIENTO', 'DOSCIENTOS', 'TRESCIENTOS', 'CUATROCIENTOS', 'QUINIENTOS', 'SEISCIENTOS', 'SIETECIENTOS', 'OCHOCIENTOS', 'NOVECIENTOS'][n];

  const procesarTresDigitos = (n) => {
    let output = [];
    if (n === 100) return 'CIEN';
    if (n > 100) {
      let c = Math.floor(n / 100);
      output.push(centenas(c));
      n %= 100;
    }
    if (n > 0 && n < 10) {
      output.push(unidades(n));
    } else if (n >= 11 && n <= 19) {
      output.push(especiales(n));
    } else if (n >= 10) {
      let d = Math.floor(n / 10);
      let u = n % 10;
      if (d === 1 && u === 0) output.push('DIEZ');
      else if (d === 2 && u === 0) output.push('VEINTE');
      else if (d === 2 && u > 0) output.push('VEINTI' + unidades(u));
      else {
        if (u > 0) output.push(decenas(d) + ' Y ' + unidades(u));
        else output.push(decenas(d));
      }
    }
    return output.join(' ');
  };
  ...
  // Millones
  if (entero >= 1000000) {
    let mill = Math.floor(entero / 1000000);
    if (mill === 1) partes.push('UN MILLÓN');
    else partes.push(procesarTresDigitos(mill) + ' MILLONES');
    entero %= 1000000;
  }
  ...
  if (entero >= 1000) {
    ...
    else partes.push(procesarTresDigitos(mil) + ' MIL');
    entero %= 1000;
  }
  
  // Centenas/Unidades
  if (entero > 0) {
    partes.push(procesarTresDigitos(entero));
  }

  let resultado = partes.join(' ');
  
  // Ajuste gramatical para pesos si termina exactamente en millones
  if (resultado.indexOf('MILLÓN') !== -1 && resultado.trim().endsWith('NES')) {
     resultado += ' DE PESOS';
  } else if (resultado === 'UN MILLÓN') {
     resultado += ' DE PESOS';
  } else {
     resultado += ' PESOS';
  }
